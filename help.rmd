...
This Shiny application is an interactive interface to parts of the R package **secrdesign** ([Efford 2025a](#references)). General context is provided by the study design chapter of the online [SECR book] ([Efford 2025b](#references)).

The application focuses on the effect of detector layout and other design variables on the precision of density estimates, measured by the relative standard error (RSE, aka CV). Vary settings on the [Design](#designspecs) page to obtain an immediate "rule-of-thumb" approximation to RSE($\hat D$), based on the expected numbers of recaptures and distinct individuals ([Efford and Boulanger 2019](#references)). The current source code is available from https://GitHub.com/MurrayEfford/secrdesignapp.
...
Options for detector type follow **secr** ([Efford 2025c](#references)): "multi" refers to a multi-catch trap; "proximity" to binary proximity detectors, and "count" to detectors that yield integer counts. Counts are assumed to be Poisson-distributed.
<br>
...
<!-- <br> -->
<!-- The "single" detector type may also be selected, but many results cannot be computed because expressions are not available for \\(\small \mathrm E (n)\\) and \\(\small \mathrm E ( r )\\). Simulation uses the likelihood for "multi" detectors. -->
<br>
...
GIS operations use **sf** internally ([Pebesma 2018](#references)).

See Options | Detector array to control the clipping of detector clusters that cross the boundary of the region.

#### Random

Two algorithms are offered for random placement - a simple random sample of points (SRS) or a balanced spatial sample (GRTS) as described by [Stevens and Olsen (2004)](#references) and implemented in package **spsurvey** ([Kincaid and Olsen 2018](#references)).
...
The latest addition to secrdesignapp is optimization of detector layouts using a genetic algorithm as implemented in the R package **kofnGA** ([Wolters 2015](#references)). The **criterion** may be either the lesser of the expected numbers of individuals or recaptures (min(n,r); [Efford and Boulanger 2019](#references), [Durbach et al. 2021](#references)) or the number detected at two or more detectors (n2; cf [Dupont et al. 2021](#references)). The 'min(n,r)' criterion is the default, and is usually preferred.
...
<table style="width:50%">
  ...
  <tr>
    ...
    <td> \( 100 . \mathrm{CF} / \sqrt { \mathrm{min} (n,r)} \) </td>
  </tr>
</table> 
<br>
...
The relative standard error RSE (aka CV) is a measure of estimator precision. \\(\small \mathrm{RSE}(\hat D) = \mathrm{SE} (\hat D) / \hat D \\). For a particular design the precision may be predicted approximately from the expected values of \\(n\\) and \\(r\\) -- this is the "rule-of-thumb" RSE. The approximation is intended for Poisson-distributed $n$ (see [Distribution](#distribution)). Sampling variance is predictably less when $n$ is binomial; the binomial $\mathrm{RSE}_B$ for a given Poisson $\mathrm{RSE}_P$ depends on the effective sampling area $a$ and total mask area $A$ as follows
$$\mathrm{RSE}_B = \sqrt {\mathrm{RSE}_P^2 - 1 / (DA)}.$$
...
Probability of detecting a change in density in a 2-survey comparison as a function of effect size for a given expected \\(\small \mathrm{RSE}(\hat D)\\). See [Efford and Boulanger 2019](#references) for the computation method.
...
The log attempts to capture key messages, warnings and errors that would otherwise be sent to the console and 
not be seen by the user. Coverage is patchy.
...
2. The default number is often enough for \\(\small \mathrm{RSE}(\hat D)\\), but many more replicates are needed to estimate of \\(\small \mathrm{RB}(\hat D)\\) reliably. <br>
...
<table style="width:50%">
  ...
  <tr>
    <td> Simulated \(\small \widehat{\mathrm{RSE}}\) </td>
    <td>mean and SE over replicates</td>
  </tr>
  <tr>
...
1. The number of moves is reported consistently with E(m) only for trap (multi) detectors. <br>
2. Estimate of true RSE from variation in \\(\small \hat D\\) over replicates \\( \small \mathrm{SD}(\hat D / D) \\). <br>
3. Relative bias \\( \small \mathrm{RB}(\hat D) = (\hat D - D) / D \\). <br>
...
This page evaluates the effect of detector spacing on precision and total cost. The array geometry is fixed on the [Design](#designspecs) page. A numerical search is performed for the array spacing that minimises the rule-of-thumb \\(\small \mathrm{RSE}(\hat D) \\).
...
Efford, M. G. (2025c) secr: Spatially explicit capture--recapture models. 
R package version 5.3.0. https://CRAN.R-project.org/package=secr

Efford, M. G. and Boulanger, J. (2019) Fast evaluation of study designs for spatially explicit capture&ndash;recapture. *Methods in Ecology and Evolution* **10**, 1529--1535. https://doi.org/10.1111/2041-210X.13239
...
[Efford and Boulanger 2019]: https://DOI.org/10.1111/2041-210X.13239

[secrdesign-manual.pdf]: https://www.otago.ac.nz/density/pdfs/secrdesign-manual.pdf
[secrdesign-Enrm.pdf]: https://www.otago.ac.nz/density/pdfs/secrdesign-tools.pdf
[secrdesign-tools.pdf]: https://www.otago.ac.nz/density/pdfs/secrdesign-tools.pdf
