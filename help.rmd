...
IMPORTANT NOTE: Maximising precision is only one component of study design. Sampling must also be representative of the population in the region of interest. Linear arrays are unreliable when home ranges are elongated in a common direction ([Efford 2018b](#references)).
...
<br>
Options for detector type follow **secr** ([Efford (2018c)](#references)): "multi" refers to a multi-catch trap; "proximity" to binary proximity detectors, and "count" to detectors that yield integer counts. Counts are assumed to be Poisson-distributed.
<br>

The detector layout may be specified as a rectangular grid or line (via `make.grid`), or read from a file. For a rectangular grid the options are:
...
<!-- A further option 'Pgrid' constructs a grid within a polygon. The polygon boundary is input from a file. -->
...
<table style="width:40%">
  ...
  <tr>
    <td>Clusters</td>
    <td>number of replicate identical arrays</td>
  </tr>
  <tr>
    <td>Distribution</td>
...
<table style="width:40%">
  ...
  <tr>
    <td>Suggest spacing</td>
    <td>Finds spacing with E(n) = E(r) and sets this for current grid or line</td>
  </tr>
  <tr>
    <td>Simulate</td>
...
'Suggest spacing' does not apply when the detector array is input from a file.
...
5\. \(k\) tends to fall in the range 0.3--1.1 (see [Efford et al. 2016](#references) for some examples). Scenarios with $\sigma$ and $D$ that result in $k$ outside this range may be unrealistic. <br>
...
Detector layout as input from file or constructed with `make.grid`
...
The 'SumSpacing' option determines route length by summing inter-detector distances.
...
with adjustments for the maximisation method (default "none"), detector type (default "proximity") and fitting function (default `openCR.fit`). If the predicted time exceeds 0.2 minutes a dialogue box allows the user to cancel simulations.
...
**secrdesign** >= 2.5.5 allows a choice between two functions for fitting closed-population SECR models: `secr.fit` in package **secr** and `openCR.fit` in package **openCR** ([Efford 2018d](#references)). The default is to use the faster multi-threaded code in `openCR.fit` (for this it is best to leave ncores = 1).
...
The table on this page has one row for each scenario; rows are added by clicking the 'Add to summary' button on the [Design](#designspecs) page, or automatically from [Simulation](#simspecs). Columns from "simfn" onwards will have missing values if no simulation has been performed.

The table may be downloaded as a comma-separated text file.
...
Efford, M. G. (2018b) Non-circular home ranges and the estimation of population density. *Ecology* (in press).

Efford, M. G. (2018c) secr: Spatially explicit capture--recapture models. R package version 3.1.8. https://CRAN.R-project.org/package=secr

Efford, M. G. (2018d) openCR: Open population capture--recapture models. R package version 1.3.0. https://CRAN.R-project.org/package=openCR
